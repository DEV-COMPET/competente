...
export default new Modal({
  ...
  run: async ({ client, interaction }) => {
    const createMemberUrl = "http://localhost:4444/competianos/"
    const nome = interaction.fields.getTextInputValue(nameInput.data.custom_id || "")
    const email = interaction.fields.getTextInputValue(emailInput.data.custom_id || "")
    const linkedin = interaction.fields.getTextInputValue(linkedinInput.data.custom_id || "")
    const url_image = interaction.fields.getTextInputValue(imageUrlInput.data.custom_id || "")
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription("Abaixo você encontra as informações sobre o usuário criado")
        ...
        .addFields(
          ...
          { name: "Linkedin", value: data.linkedin || " Nenhum linkedin informado", inline: false },
      ...
      embed.setTimestamp()
      await interaction.reply({ content: "Seu envio foi realizado com sucesso!", ephemeral: true, embeds: [embed] })
    }
    else {
      ...
      const embed = new EmbedBuilder()
        ...
        .setAuthor({ name: interaction.user.username.replaceAll("_", " ") || "abc", iconURL: interaction.user.avatarURL() || undefined })
        ...
        .addFields(
          { name: "Código do erro", value: data.code.toString(), inline: false },
          { name: "Mensagem do erro", value: data.message, inline: false },
        )
      await interaction.reply({ content: "Não foi possível concluir o cadastro", ephemeral: true, embeds: [embed] })
    }
  }
})
export { modal }