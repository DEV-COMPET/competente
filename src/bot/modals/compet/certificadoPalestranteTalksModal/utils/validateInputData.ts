...
export async function validateInputData({ palestrantes, data, minutos }: ExtractInputDataResponse): Promise<ValidateInputDataResponse> {
    ...
    if(!validateSpeakers(palestrantes))
        errorMessage += "Os nomes dos palestrantes devem estar no formato correto, separados por ponto e vírgula.\n";
    ...
    const speakersArray = getSpeakers(palestrantes);

    return right({ inputData: { palestrantes: speakersArray, data: dataString, minutos: minutesString } });
}

function validateSpeakers(palestrantes: string): boolean {
    // Verifica se está vazio
    if (!palestrantes || palestrantes.trim() === "") return false;

    // Divide os nomes dos palestrantes por ponto e vírgula
    const palestrantesList = palestrantes.split(";");

    // Verifica se cada nome está em formato adequado (sem caracteres inválidos e não vazio)
    const nameRegex = /^[A-Za-zÀ-ÖØ-öø-ÿ\s]+$/; // Aceita letras com acentos e espaços
    for (const palestrante of palestrantesList) {
        const trimmedName = palestrante.trim();
...
function getSpeakers(palestrantes: string): string[] {
    return palestrantes.split(";").map(p => p.trim());
}
