...
async function getEmails(): Promise<GetEmailsResponse> {
    ...
    const emails = [...new Set(registrations.value.certificados.map(certificado => certificado.email).filter(Boolean))];
    return right({ emails })
}