...
import { generatePDFMultiplePages } from "@/bot/selectMenus/talksSpeakerCertificate/utils/pdf/multiplePagesSpeakersPDF";
...
import { palestrantesArray, datasArray, minutosArray } from "@/bot/modals/compet/certificadoPalestranteTalksModal/certificadoPalestranteTalksModal";
...
const { customId, label } = readJsonFile({
    dirname: __dirname,
    partialPath: "confirmButtonTalksSpeakersCertificateInput.json"
});

export const confirmButtonTalksSpeakersCertificate = makeSuccessButton({ customId, label });
...
export default new Button({
    ...
    run: async ({ interaction }) => {
        ...
        const nomeSaida = talksName[talksName.length - 1] + ' - ' + datasArray[datasArray.length - 1] + ' - Certificados Palestrantes';
        ...
        await generatePDFMultiplePages(palestrantesArray[palestrantesArray.length-1], eventType, talksName[talksName.length - 1],
        ...
        await uploadToFolder(`${pdfPath}.pdf`, env.GOOGLE_DRIVE_TALKS_SPEAKERS_FOLDER_ID);
